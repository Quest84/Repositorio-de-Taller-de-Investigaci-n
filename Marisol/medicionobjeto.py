...
def measure(img):
    ...
    for cnt in contours:
        ...
        M = cv.moments(cnt)
        # Obtenga el rectángulo circunscrito del contorno, x, y son las coordenadas de píxeles de la esquina
        #superior izquierda del marco verde, w, h son la longitud y el ancho del marco verde
        ...
        if M['m00'] != 0:
            ...
            cx = int(M['m10'] / M['m00'])
            cy = int(M['m01'] / M['m00'])
            #Según el punto central obtenido por la distancia geométrica, dibuje el círculo central,
            #que está bloqueado por la línea azul, por lo que no se puede ver
            ...
            for (x, y) in box:
                ...
                # tl coordenadas del número de la imagen de la esquina superior izquierda, tr coordenadas del número de la imagen de la esquina superior derecha,
                #br coordenadas del número de la imagen de la esquina inferior derecha, bl coordenadas del número de la imagen de la esquina inferior izquierda
                (tl, tr, br, bl) = box
                ...
                cv2.line(img, (int(tltrX), int(tltrY)), (int(blbrX), int(blbrY)),(255, 0, 0), 2)
                cv2.line(img, (int(tlblX), int(tlblY)), (int(trbrX), int(trbrY)),(255, 0, 0), 2)
...
while(cap.isOpened()):
    ...
    if cv2.waitKey(1) & 0xFF == ord('p'):
        cv2.imwrite("C:/Users/maris/Documents/ESCUELA TESJO/8 OCTAVO SEMESTRE/TALLER DE INVESTIGACION II/PROYECTO/Taller_de_Investigacion/Marisol/img1.png", img)
        break
